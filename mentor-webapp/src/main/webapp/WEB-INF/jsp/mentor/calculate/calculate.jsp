...
mentor.PageNavi = React.render(
    <PageNavi url={mentor.contextpath + "mentor/calculate/ajax.listCalculateLectureByBelongMentor.do"} pageFunc={fn_search} totalRecordCount={0} currentPageNo={1} recordCountPerPage={5} contextPath={'${pageContext.request.contextPath}'}/>,
    document.getElementById('divPaging')
);

mentor.smsPageNavi = React.render(
    <PageNavi url={mentor.contextpath + "mentor/calculate/ajax.listCalculateSmsSendResult.do"} pageFunc={fn_search4Sms} totalRecordCount={0} currentPageNo={1} recordCountPerPage={5} contextPath={'${pageContext.request.contextPath}'}/>,
...
<script type="text/javascript">
    ...
    var fn_search = function(pageNum){
        ...
        $.ajax({
            url: "${pageContext.request.contextPath}/mentor/calculate/ajax.listCalculateLectureByBelongMentor.do",
    ...
    var fn_search4Sms = function(pageNum){
        ...
        dataSet.params.searchStDate = $("#searchStartDate4Sms").val().replace(/-/gi, "");
        dataSet.params.searchEndDate = $("#searchEndDate4Sms").val().replace(/-/gi, "");
        dataSet.params.posCoYn = "Y";

        $.ajax({
            url: "${pageContext.request.contextPath}/mentor/calculate/ajax.listCalculateSmsSendResult.do",
            data : $.param(dataSet.params, true),
            success: function(rtnData) {
    ...
    $("#aExcelDown").click(function(e){
        ...
        var url = "${pageContext.request.contextPath}/mentor/calculate/ajax.excelDownListCalculate.do";
    ...
    $("#bExcelDown").click(function(e){
        ...
        var url = "${pageContext.request.contextPath}/mentor/calculate/ajax.excelDownSmsSendResult.do";
        var paramData = $.param(dataSet.params, true);
        var inputs = "";
        jQuery.each(paramData.split('&'), function(){
            var pair = this.split('=');
