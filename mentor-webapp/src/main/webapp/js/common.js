...
var Studio = function(start,end){

    this.starts = [];
    this.ends = [];

    start = new Date(start.substring(0,4),parseInt(start.substring(4,6))-1,start.substring(6,8),start.substring(8,10),start.substring(10,12));
    end = new Date(end.substring(0,4),parseInt(end.substring(4,6))-1,end.substring(6,8),end.substring(8,10),end.substring(10,12));

    this.starts.push(start.valueOf());
    this.ends.push(end.valueOf());

    this.contains = function(t1,t2){
        t1 = (new Date(t1.substring(0,4),parseInt(t1.substring(4,6))-1,t1.substring(6,8),t1.substring(8,10),t1.substring(10,12))).valueOf();
        t2 = (new Date(t2.substring(0,4),parseInt(t2.substring(4,6))-1,t2.substring(6,8),t2.substring(8,10),t2.substring(10,12))).valueOf();
        for(var idx = 0; idx < this.starts.length; idx++){
            if(t1 >= this.starts[idx] && t1 <= this.ends[idx]){
                return true;
            }
        }
        for(var idx = 0; idx < this.starts.length; idx++){
            if(t2 >= this.starts[idx] && t2 <= this.ends[idx]){
                return true;
            }
        }
        return false;
