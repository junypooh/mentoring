...
function showSubMenu(obj){
    //$('.sub-gnb','#header').hide();$(obj).next().show();
    $(obj).closest('li').siblings()
...
function initCode(target,supCd,initVal, callback){
    $.ajax({
        url: mentor.contextpath+"/ajax.code.do",
        data : {"supCd":supCd},
        success: function(rtnData) {
            target.loadSelectOptions(rtnData,initVal,"cd","cdNm",1);
...
function _nvl(str){
    var rtn = arguments[1]?arguments[1]:"";
    if(str){return str;}return rtn;
}
