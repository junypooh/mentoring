...
<script type="text/javascript">
    ...
    <div class="search-area general-srch">
        <ul>
            <li class="condition-big">
                ...
                <ul class="detail-condition" id="rdoMbrCualfCds">
                    <li>
                        <label><input type="radio" name="mbrCualfCds" value="" <c:if test="${empty param.mbrCualfCds}">checked='checked'</c:if> /> 전체</label>
                    </li>
                </ul>
            </li>
        </ul>
        ...
        <ul>
            <li class="condition-big">
                <p><strong>보호자 동의</strong></p>
                <ul class="detail-condition">
                    <li>
                        <label><input type="radio" name="approveYn" value="" <c:if test="${empty param.approveYn}">checked='checked'</c:if> /> 전체</label>
                    </li>
                    <li>
                        <label><input type="radio" name="approveYn" value="Y" <c:if test="${param.approveYn eq 'Y'}">checked='checked'</c:if> /> 동의</label>
                    </li>
                    <li>
                        <label><input type="radio" name="approveYn" value="N" <c:if test="${param.approveYn eq 'N'}">checked='checked'</c:if> /> 동의안함</label>
                    </li>
                </ul>
            </li>
        </ul>
        <ul>
            <li class="condition-big">
                <p><strong>반대표</strong></p>
                <ul class="detail-condition">
                    <li>
                        <label><input type="radio" name="rpsStudent" value="" <c:if test="${empty param.rpsStudent}">checked='checked'</c:if> /> 전체</label>
                    </li>
                    <li>
                        <label><input type="radio" name="rpsStudent" value="Y" <c:if test="${param.rpsStudent eq 'Y'}">checked='checked'</c:if> /> 대표 (O)</label>
                    </li>
                    <li>
                        <label><input type="radio" name="rpsStudent" value="N" <c:if test="${param.rpsStudent eq 'N'}">checked='checked'</c:if> /> 대표아님 (X)</label>
                    </li>
                </ul>
            </li>
        </ul>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        $.each(userCategory, function(k, v) {
            if('${param.mbrCualfCds}' == k) {
                mbrCualfCds.push($('<li><input type="radio" name="mbrCualfCds" value="' + k + '" checked="checked" /> ' + v + '</label></li>'));
            } else {
    ...
    function loadData(curPage, recordCountPerPage){
        ...
        var _param = jQuery.extend({'regDtmBegin':$("#regDtmBegin").val()
                      ...
                      ,'rpsStudent':$(':radio[name="rpsStudent"]:checked').val()
                      ...
                      ,'school':$('#school').val()
                      ,'clas':$('#clas').val()
                      ,'agree':$(':radio[name="agree"]:checked').val()
                      ,'captain':$(':radio[name="captain"]:checked').val()}, dataSet.params);

        $.ajax({
            url: "${pageContext.request.contextPath}/member/public/general/ajax.list.do",
