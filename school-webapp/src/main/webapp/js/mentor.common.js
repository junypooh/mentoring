...
//         if(arguments[0].currentTarget.files[0].size>20*1024*1024){
 ...
 * Byte 단위로 글자 를 잘라냄
 */
String.prototype.substrBytes = function(start, length) {
    var size = 0, offset = 0;
...
function _fCallTomms(sid,classSer)
{
    $.ajax({
        url: mentor.contextpath+"/ajax.encriptBase64.do",
        data : {"sid":sid,"classSer":classSer},
...
mentor.parseTime = function(time){
        ...
        return this.emptySelect(remainOption).each(function(){
            if (this.tagName=='SELECT') {
                ...
                $.each(optionsDataArray,function(index,optionData){
                    var getObjVal = function(_data, _target){
                        var val = "";
                    ...
                    var itemVal = getObjVal(optionData,code);
                    var itemTxt = getObjVal(optionData,name);

                    var selected = optionData[code]==selectCode?' selected':'';
                    var option = '<option value="{0}" {1}>{2}</option>'.format(itemVal,selected, itemTxt);
                    $(selectElement).append(option);
                });
            }
        });
    };
    //select의 options을 추가 etc추가
    $.fn.loadSelectOptionsEtc = function(optionsDataArray, selectCode, code, name, remainOption, etc) {
        if(typeof(remainOption) === "undefined"){
                remainOption = 0;
        }
        return this.emptySelect(remainOption).each(function(){
            if (this.tagName=='SELECT') {
                ...
                $.each(optionsDataArray,function(index,optionData){
                    ...
                    var itemVal = getObjVal(optionData,code);
                    var itemTxt = getObjVal(optionData,name);
                    var itemEtc = getObjVal(optionData,etc);

                    var selected = optionData[code]==selectCode?' selected':'';
                    var option = '<option value="{0}" {1} {3}>{2}</option>'.format(itemVal,selected, itemTxt, 'data=' + itemEtc);
                    $(selectElement).append(option);
                });
            }
        });
...
function initCode(target,supCd){
    $.ajax({
        url: mentor.contextpath+"/ajax.code.do",
        data : {"supCd":supCd},
        success: function(rtnData) {
            target.loadSelectOptions(rtnData,"","cd","cdNm",0);
        }
    });
}
...
/**
 * PC와 모바일 구분하여 PC에서는 TOMMS 실행,
 * 모바일이라면 어플설치여부 확인하여 설치되어 있으면 어플 실행
 * 설치가 안되어 있다면 설치안내팝업 호출
 * @param sid
 */
function solutionStart(sid,classSer){
    if(mentor.isMobile){ //모바일 체크
        if(isAppInstalled()){ //어플 설치여부 확인
            //설치여부 true : 어플 실행
            appStart();
        }else{
            //설치여부 false : 설치안내 팝업
        ...
        _fCallTomms(sid,classSer); //영상솔루션 실행
    }
}
 ...
 * 어플 설치안내 팝업 호출
 */
function popupAppInstallInfo(){
    if($.isEmptyObject($.find("#divAppInfo"))){
        $("#wrap").append($("<a/>", {href : "#layerAppInfo", class:"layer-open", id:"aAppInfo"}));
        $("#wrap").append($("<div/>", {id : "divAppInfo"}));
    }

    $("#divAppInfo").load(mentor.contextpath + "/layer/layerPopupAppInstallInfo.do", function(){
        position_cm();
        $("#aAppInfo").trigger("click");
    });
}
